...
def load_data_olivettiface():
    ...
    std = img.std()
    img = (img - mean) / std
    return train_test_split(img,target,test_size=0.2,random_state=42,shuffle=True)
...
class OlivettiDataset(Dataset):
    def __init__(self,x,y, toTensor=transforms.ToTensor()):
    ...
    for epoch in range(Epochs):
        ...
        for trainX , trainY in train_data:
            ...
            if total_train_step % 5 == 0:
                end_time = time.time()
                print(f'Epoch:{epoch + 1}/{Epochs} , 训练次数:{total_train_step} '
                      f', Loss:{loss.item():.6f}, 耗时: {(end_time - start_time):.1f} 秒')
